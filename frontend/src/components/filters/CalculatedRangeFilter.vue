<template>
  <div class="calculated-range-filter">
    ...
    <!-- Informacja o obliczanym filtrze -->
    <div
      v-if="showInfo"
      class="p-2 bg-gray-50 rounded-md text-xs text-gray-600 mb-3"
    >
      <div class="flex items-start">
        <IconInformation
          class="w-4 h-4 mr-1 text-gray-500 flex-shrink-0 mt-0.5"
        />
        <div>{{ infoText }}</div>
      </div>
    </div>
...
const props = defineProps({
  ...
  infoText: {
    type: String,
    default:
      "Ten filtr jest obliczany na podstawie innych parametrów samochodu.",
  },
});
